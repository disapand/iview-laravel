...
class CreateOnlineResourcesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('online_resources', function (Blueprint $table) {
            $table->increments('id');
            $table->string('name', 50)->default('媒体名称')->comment('媒体名称');
            $table->string('category', 20)->default('网站')->comment('类别');
            $table->string('form', 20)->default('横幅广告')->comment('形式');
            ...
            $table->string('platform', 20)->default('综合')->comment('媒体内容属性类别');
