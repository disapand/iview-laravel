...
class CreateInternetcelebrityResourcesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('internetcelebrity_resources', function (Blueprint $table) {
            $table->increments('id');
            $table->string('name', 50)->default('网名（本名），如不知道本名仅写网名即可，网名和本名相同写一个即可')->comment('KOL名称')->nullable();
            $table->string('platform', 20)->default('Facebook')->comment('媒体内容属性类别')->nullable();
            ...
            $table->integer('fans')->default(0)->comment('粉丝数量最多的平台的粉丝量')->nullable();
            ...
            $table->string('cooperation')->default('合作过的品牌举例')->comment('合作品牌')->nullable();
