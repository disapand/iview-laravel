...
class CreateTransformResourcesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('transform_resources', function (Blueprint $table) {
            ...
            $table->string('position', 50)->default('整车喷绘、车体两侧、车头、车尾、车门、车厢内等等')->nullable()->comment('媒体具体位置');
            $table->string('language', 20)->default('中文')->nullable()->comment('语言')->nullable();
            $table->string('category', 20)->default('飞机')->nullable()->comment('类别');
            $table->string('area', 50)->default('媒体所在交通工具的线路或区域')->nullable()->comment('覆盖区域');
            $table->string('SOV', 20)->default('1/4')->nullable()->comment('SOV');
            $table->integer('visitor')->default(0)->nullable()->comment('人流量')->nullable();
            $table->integer('traffic')->default(0)->nullable()->comment('车流量')->nullable();
            $table->integer('minimum_buy')->default(0)->nullable()->comment('最小采购周期或单位')->nullable();
            $table->string('format', 100)->default('文字+数字')->nullable()->comment('规格尺寸');
            $table->string('number', 100)->default('一个单位位置的广告面数，如：一个位置2面、4面等')->nullable()->comment('广告载体面数');
