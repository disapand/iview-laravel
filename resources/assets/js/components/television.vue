    ...
    <div>
        ...
        <div style="margin: 20px 0; display: flex; align-items: center">
            <div>
                ...
                <Button @click="handleSelectAll(false)" v-if="deleteAll">取消</Button>
                <Button @click="handleUsrNone" v-if="deleteAll" type="warning">取消发布选中</Button>
                <Button @click="handleUsrAll()" v-if="deleteAll" type="success">发布选中</Button>
        ...
        <div>
            <i-table ref="tvs" border :columns="col" :data="tvs" stripe :highlight-row=false @on-selection-change="selected"
...
<script>
    export default {
        ...
        mounted() {
            setTimeout( () => {
                this.$ajax.get('https://www.zetin.cn/api/countTelevison/' + this.$store.state.user.id)
                    .then(res => {
                        console.log(res)
                        this.count = res.data
                    })
        ...
        methods: {
            ...
            handleSelectAll(status) {
                this.$refs.tvs.selectAll(status)
            },
            handleRouter(id) {
            ...
            deleteSelection () {
                ...
                this.$ajax.delete( window.location.href.substring(0, window.location.href.indexOf(window.location.pathname)) +
                    ...
                    .catch( (error) => {
                        console.log('电视资源批量删除出错', error)
                        this.$Message.error('批量删除出现异常')
                        this.$Loading.error()
                    })
            },
            handleUse( params ) {
                ...
                this.$ajax.get(window.location.href.substring(0, window.location.href.indexOf(window.location.pathname)) +
                    '/api/tvUse/' + params.row.id)
                    .then( (response) => {
                        console.log('返回值：', response)
                        params.row.isuse = ! params.row.isuse
                        this.$Message.success('资源发布/取消发布成功')
                        this.$Loading.finish()
                    })
                    .catch( (error) => {
                        console.log('电视资源发布失败', error)
                        this.$Message.error('发布/取消发布异常')
                        this.$Loading.error()
                    })
            },
            handleUsrAll () {
