...
<script>
    export default {
        ...
        created() {
            this.$ajax.get('http://www.zetin.cn/api/dynamic').then((response) => {
                ...
                if (this.$route.params.currentPage == undefined || this.$route.params.currentPage == 1) {
                    ...
                    if (this.condition && this.search) {
                        uri = 'http://www.zetin.cn/api/dynamic/' + this.condition + '/' + this.search + '?page=' + this.total
                    } else {
                        uri = 'http://www.zetin.cn/api/dynamic?page=' + this.total
                    }
                    ...
                    let uri
                    if (this.condition && this.search) {
                        uri = 'http://www.zetin.cn/api/dynamic/' + this.condition + '/' + this.search + '?page=' + this.$route.params.currentPage
                    } else {
                        uri = 'http://www.zetin.cn/api/dynamic?page=' + this.$route.params.currentPage
                    }
                    this.$ajax.get(uri).then((response) => {
                        this.dynamic = response.data.data
                        this.loading = false
        ...
        methods: {
            ...
            searchDynamic() {
                ...
                this.$ajax.get('http://www.zetin.cn/api/dynamic/' + this.condition + '/' + this.search).then((response) => {
                    this.dynamic = response.data.data
                    this.total = response.data.meta.pagination.total
                    this.all = true
