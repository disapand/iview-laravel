        ...
        methods: {
            ...
            publishOrCancel( params ) {
                ...
                this.$ajax.get(window.location.href.substring(0, window.location.href.indexOf(window.location.pathname)) +
                    '/api/publishNewspaper/' + params.row.id )
                    .then( (response) => {
            ...
            cancelSelection () {
                this.$Loading.start()
                this.$ajax.get(window.location.href.substring(0, window.location.href.indexOf(window.location.pathname)) +
                    '/api/cancelSelectionNewspaper/' + this.selectedList.join('-') )
                    .then( (response) => {
                        for( let i = 0; i < this.selectedList.length; i ++) {
                            for ( let j = 0; j < this.newspaper.length; j ++) {
                                if (this.newspaper[j].id == this.selectedList[i]) {
                                    this.newspaper[j].isuse = false
                                    break
                                }
                            }
                        }
                    ...
                    .catch( (error) => {
                        console.log('批量取消发布纸媒资源出错：', error)
            ...
            publishSelection () {
                this.$Loading.start()
                this.$ajax.get(window.location.href.substring(0, window.location.href.indexOf(window.location.pathname)) +
                    '/api/publishedSelectionNewspaper/' + this.selectedList.join('-') )
                    .then( (response) => {
                        for( let i = 0; i < this.selectedList.length; i ++) {
                            for ( let j = 0; j < this.newspaper.length; j ++) {
                                if (this.newspaper[j].id == this.selectedList[i]) {
                                    this.newspaper[j].isuse = true
                                    break
                                }
                            }
                        }
